...
namespace SYNARMOSMA {
  ...
  class Geometry {
   ...
   public:
    ...
    /// This method computes the product of the matrix \f$L\f$ that is its first argument and the matrix of coordinates, \f$A_{ij} = \sum_k L_{ik} x_k(j)\f$ where \f$i\f$ ranges over the number of rows in \f$L\f$ and \f$j\f$ over Geometry::background_dimension. The method then contracts this intermediate matrix \f$A\f$ with the coordinate matrix, skipping those vertices with the value -1 in the method's second argument, an offset vector, and returns the scalar value of this contraction.
